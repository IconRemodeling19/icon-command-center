...
if (summary) { text = summary; } else if (desc) { text = desc.length > 100 ? desc.slice(0, 100) + '...' : desc; }
} else {
const source = summary || desc;
if (source) { text = source.length > 80 ? source.slice(0, 80) + '...' : source; }
}
if (!text) return null;
return (
...
document.dispatchEvent(new MouseEvent('mousemove', { bubbles: true, cancelable: true, view: window }));
...
const interval = setInterval(() => { videos.forEach(v => { if (v.paused) v.play().catch(() => {}); }); }, 10000);
return () => clearInterval(interval);
}, []);
...
<video style={{ position: 'fixed', opacity: 0, width: '1px', height: '1px', top: 0, left: 0, pointerEvents: 'none' }} autoPlay loop muted playsInline>
<source src="data:video/mp4;base64,AAAAIGZ0eXBpc29tAAACAGlzb21pc28yYXZjMW1wNDEAAAAIZnJlZQAAAuFtZGF0" type="video/mp4" />
</video>
<div style={{ position: 'absolute', inset: 0, pointerEvents: 'none', opacity: 0.04, backgroundImage: 'linear-gradient(#fff 1px, transparent 1px), linear-gradient(90deg, #fff 1px, transparent 1px)', backgroundSize: '60px 60px' }} />
...
{TEAM.map((p) => (
<PersonColumn key={p.id} person={p} tasks={tasksByPerson[p.id] || []} scale={scale} />
))}
</main>
<footer style={{
position: 'relative', flexShrink: 0,
